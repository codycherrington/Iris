...
| `liquid-glass-api.md` | Where the macOS 26 Liquid Glass API actually lives, its surface, why `glassEffectID`/`glassEffectUnion` define Iris's animation language — where fusion turned out to be the wrong default, and (2026-08-09) why `.scaleEffect` outside `glassEffect` slides content inside its own shape. |
